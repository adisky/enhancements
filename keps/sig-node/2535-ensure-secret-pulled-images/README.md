...
Currently with Image pull policy `IfNotPresent` there is security risk that an 
unauthorized pod can access a image pulled with image pull secret.

We are proposing to modify the behavoir of `IfNotPresent` image pull policy. It
will ensure that images pulled with `imagePullSecrets` are re-authenticated or 
access denied in following scenerios:

1. When a pod that does not contain same image pull secret as the first image pull
   wants to use that image.
2. When a pod that does not contain any image pull secret wants access that image.

This will have no affect on the `pull never` and `pull always` image pull
